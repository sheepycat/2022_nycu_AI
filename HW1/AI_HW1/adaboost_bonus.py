        ...
        for f,feature in zip(featureVals,features):
          ...
          for data_index in range(len(f)):
            if (f[data_index] < 0):
              ...
              h=0
            e.append(np.square((abs(h-labels[data_index])*weights[data_index]),2))
            
          feature_count +=1
          print(cur_error,type(cur_error))
          e = np.sum(e,dtype = np.float64)
          arr = np.sqrt(e)
          cur_error = arr[0]
          #if(type(cur_error) != np.float64):
            #cur_error = cur_error.real()
          print(cur_error,type(cur_error))
